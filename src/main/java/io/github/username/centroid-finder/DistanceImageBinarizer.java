...
public class DistanceImageBinarizer implements ImageBinarizer {
    ...
    public int[][] toBinaryArray(BufferedImage image) {
        ...
        for (int y = 0; y < binaryArray.length; y++) {
            for (int x = 0; x < binaryArray[0].length; x++) {
                int imageColor = (image.getRGB(x, y) & 0x00FFFFFF);
    ...
    public BufferedImage toBufferedImage(int[][] image) {
        BufferedImage photo = new BufferedImage(image[1].length, image.length, BufferedImage.TYPE_INT_RGB);
        for(int y = 0; y < image.length; y++) {
            for(int x = 0; x < image[y].length; x++) {
                if(image[y][x] == 1) {
                    photo.setRGB(x, y, 0xFFFFFF);
                } else {
                    photo.setRGB(x, y, 0x000000);
                }
            }
        }
        return photo;
